  ...
  <nav class="bg-white border-b border-gray-200 px-4 sm:px-5 py-2.5 md:h-16">
    <div class="flex flex-wrap items-center">
      ...
      <div class="flex items-center md:order-2 ml-auto">
        <BellIcon class="text-gray-500 w-6 h-6 mx-6" />

        <button type="button" class="flex mr-3 text-sm bg-gray-800 rounded-full md:mr-0 focus:ring-4 focus:ring-gray-300" id="user-menu-button" aria-expanded="false" data-dropdown-toggle="dropdown">
          <span class="sr-only">Open user menu</span>
          <img class="w-8 h-8 rounded-full" src="https://picsum.photos/80" alt="user photo">
        </button>
      ...
      <div class="justify-between items-center w-full md:flex md:w-auto md:order-1"
           id="mobile-menu-2"
          :class="isMobileMenuVisible ? '' : 'hidden'">
        <ul class="flex flex-col mt-4 md:ml-10 md:flex-row md:space-x-8 md:mt-0 md:text-sm md:font-medium">
          <li v-for="item in navigation" :key="item.name">
            <a href="#" class="block py-2 pr-4 pl-3  border-b border-gray-100 hover:bg-gray-50 md:hover:bg-transparent md:border-0 md:hover:text-blue-700 md:p-0"
              :class="item.isActive ? 'text-blue-700' : 'text-gray-700'">
              {{ item.name }}
            </a>
          </li>
        </ul>
...
<script>
import { BellIcon } from "@vue-hero-icons/solid";
...
export default {
  ...
  data: () => {
    return {
      ...
      navigation: [
        { name: 'Contrats', isActive: true },
        { name: 'Clients', isActive: false },
        { name: 'Équipes', isActive: false },
        { name: 'Modèles', isActive: false },
      ]
    }
  },
}
